...
class Mentor:
    ...
    async def on_request_send(self, ctx: discord.Interaction, location, tech,
                              other):
        ...
        mentee_channel: discord.TextChannel = await self.bot.fetch_channel(
            self.mentee_channel)
        mentor_channel: discord.TextChannel = await self.bot.fetch_channel(
            self.mentor_channel)
